...
def getfa(rs, ext, pre):
    ...
    cmd = "bedtools getfasta -fi /picb/molsysbio/usr/caoyaqiang/1.Projects/17.Alu/1.GenomeReference/1.hg38/1.hg38_Sequence/hg38.fa -bed {bed} -name -fo {fa}".format(
        bed=bed, fa=pre)
    callSys([cmd, "rm %s" % bed])


def getfas():
    for f in glob("../3.Modules/*.bed"):
