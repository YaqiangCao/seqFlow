...
def preMat(fs=glob.glob("*.txt"), qcut=1e-10, cut=10):
    reps = set()
    mats = {}
    for f in fs:
        pre = f.split("_")[0]
...
def enrichPlotBubble(cmap="Blues"):
    ...
    ax.set_xticks(nx)
    ax.set_yticks(ny)
    ax.set_xticklabels(map(str, mat_qs.index), rotation=90, size=6)
...
def main():
    ...
    Parallel(n_jobs=30)(delayed(repST)(value[0], value[1], key, reps)
