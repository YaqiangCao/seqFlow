...
def iteres_stat_filter( bam,
    ...
    if not os.path.exists( os.path.join( stat_root,sample ) ):
        ...
        c1 = "iteres stat {genome_size} {subfam_size} {rmsk} {bam} -o {pre} -T".format(
            genome_size=CHROM_SIZE,subfam_size=SUBFAMILY_SIZE, rmsk=RMSK, bam=bam, pre=stat_pre
            )
        call_sys( [ c1 ] )
    if not os.path.exists( os.path.join( filter_root,sample ) ):
        ...
        c2 = "iteres filter {genome_size} {subfam_size} {rmsk} {bam} -o {pre} -T".format(
            genome_size=CHROM_SIZE,subfam_size=SUBFAMILY_SIZE, rmsk=RMSK, bam=bam, pre=filter_pre
            )
        call_sys( [ c2 ] )
...
def pipeline(  ):
    ...
    bams.sort(  )
    Parallel( n_jobs=5 )( delayed( iteres_stat_filter)( bam ,stat_root, filter_root) for bam in bams )
