...
#index gff file
INDEXS = {
    "SE": "/picb/molsysbio/usr/caoyaqiang/1.Projects/17.Alu/1.GenomeReference/9.MISO/3.Index/SE",
    "RI": "/picb/molsysbio/usr/caoyaqiang/1.Projects/17.Alu/1.GenomeReference/9.MISO/3.Index/RI",
    "A3SS": "/picb/molsysbio/usr/caoyaqiang/1.Projects/17.Alu/1.GenomeReference/9.MISO/3.Index/A3SS",
    "A5SS": "/picb/molsysbio/usr/caoyaqiang/1.Projects/17.Alu/1.GenomeReference/9.MISO/3.Index/A5SS",
    "MXE": "/picb/molsysbio/usr/caoyaqiang/1.Projects/17.Alu/1.GenomeReference/9.MISO/3.Index/MXE",
}
...
def pipeline( bam,rlen=111,rs= [ "1.ILD","2.PSI",] ):
    ...
    c1 = "pe_utils --compute-insert-len {bam} {exons} --output {out}".format( bam=bam,exons=EXONS,out=ild_root )
    call_sys( [ c1 ] )
    ...
    for key, value in INDEXS.items(  ):
        ...
        c2 = "miso --run {index} {bam} --output-dir {out} --read-len {rlen} --paired-end {mean} {std} --prefilter -p 5".format( index=value,bam=bam,out=nRoot,rlen=rlen,mean=mean,std=std )
        c3 = "summarize_miso --summarize-samples {root} {root}".format( root=nRoot )
...
def getDEIs( s1, s2, pre,rs = [ "3.DEIs","4.Filter" ] ):
    ...
    c1 = "compare_miso --compare-samples {s1} {s2} {pre}".format( s1=s1,s2=s2,pre=pre1 )
    call_sys( [ c1 ] )
    ...
    c2 = "filter_events --filter {bf} --num-total 20 --delta-psi 0.2 --bayes-factor 1 --apply-both --output-dir {pre}".format( bf=bf,pre=pre2 )
    call_sys( [ c2 ] )
...
def furthurCut( deltaPsi=0.25,bf=1,root="4.Filter" ): 
    ...
    for f in fs:
        ...
        a, b , c, d = mat[ "sample1_ci_low" ], mat[ "sample1_ci_high" ], mat[ "sample2_ci_low" ], mat[ "sample2_ci_high" ]
        g,h = b - a, d -c 
        g = g[ g<deltaPsi ]
        h = h[ h<deltaPsi ]
