...
def get10xH5Matrix(fh5, mingene=1000, mincell=10, mtratio=0.05,
                   mtprefix="mt-"):
    """
    ...
    with h5py.File(fh5, "r") as f:
        ...
        matrix = sp_sparse.csc_matrix(
            (f['matrix']['data'], f['matrix']['indices'],
             f['matrix']['indptr']),
            shape=f['matrix']['shape'])
        matrix = matrix.todense()
        gs = [
            feature_ids[i] + "|" + feature_names[i]
            for i in range(len(feature_ids))
        ]
        mat = pd.DataFrame(matrix, index=gs, columns=barcodes)
        #first filter mt genes
        mtgenes = [
            i for i in mat.index if i.split("|")[1].startswith(mtprefix)
        ]
        ss = mat.sum(axis=0)
